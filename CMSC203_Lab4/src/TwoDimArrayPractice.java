...
public class TwoDimArrayPractice extends JFrame
{
   ...
   */
   public void printArray() {
	    ...
	    for (int row = 0; row < intArray.length; row++) {
	        ...
	        for (int column = 0; column < intArray[row].length; column++) {
	            System.out.print(intArray[row][column] + " "); // print element
	            animate(row, column); // animate after each element
	        }
	        System.out.println(); // new line after each row
	    }
	    System.out.println(); // optional extra line for readability
	}
   ...
   */
   public void setValues(int value, int row) {
	    ...
	    for (int column = 0; column < intArray[row].length; column++) {
	        intArray[row][column] = value;  // set element to specified value
	        animate(row, column);           // animate the change
	    }
	}
   ...
   */
   public int findMinimum(int column) {
	    // Safety check: make sure column is valid
	    ...
	    int minimum = intArray[0][column]; // start with the first row's value in the column

	    for (int row = 0; row < intArray.length; row++) {
	        if (intArray[row][column] < minimum) {
	            minimum = intArray[row][column]; // update minimum if smaller value found
	        }
	        animate(row, column, minimum); // animate after checking each element
	    }

	    return minimum; // return the smallest value found in the column
	}
   ...
   */
   public int countFound(int value) {
	    int count = 0; // tracks frequency of the value

	    for (int row = 0; row < intArray.length; row++) {
	        for (int column = 0; column < intArray[row].length; column++) {
	            if (intArray[row][column] == value) {
	                count++; // increment if value matches
	            }
	            animate(row, column, count); // animate after checking each element
	        }
	    }

	    return count; // return total frequency
	}
